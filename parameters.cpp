    ...
    , mesh(image_width, image_height, QImage::Format_RGB32)
{
    ...
    // ====================== Частота ========================
    constexpr auto wMin = 0, wMax = 100;
    ...
    // ==================== Шаг частоты =================
    constexpr auto dwMin = 0, dwMax = 10;
    QSlider * dwS = new QSlider(Qt::Horizontal);
    dwS->setRange(dwMin, dwMax);
    connect(dwS, SIGNAL(valueChanged(int)), dwSB, SLOT(setValue(int)));
    connect(dwSB, SIGNAL(valueChanged(int)), dwS, SLOT(setValue(int)));
